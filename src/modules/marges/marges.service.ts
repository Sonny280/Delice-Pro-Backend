...
// FIX 5 : mode:"insensitive" retiré conditionnellement — incompatible avec SQLite
//         (mode desktop). PostgreSQL en a besoin (sensible à la casse par défaut),
//         SQLite non (déjà insensible à la casse par défaut sur "contains").

import prisma from "../../config/database";
import { env } from "../../config/env";

// SQLite (mode DESKTOP) ne supporte pas mode:"insensitive" — et n'en a pas
// besoin, ses recherches "contains" sont déjà insensibles à la casse par
// défaut. PostgreSQL (mode SAAS), lui, est sensible à la casse par défaut,
// donc ce paramètre y reste nécessaire pour garder le même comportement
// (trouver "Beurre" en cherchant "beurre").
const modeInsensible = env.DEPLOY_MODE === "DESKTOP" ? {} : { mode: "insensitive" as const };
...
export async function getMargesTousProduits(companyId: string) {
  ...
  const avecMarge    = marges.filter((m: any) => m.margePct !== null);
  const margeMoyenne = avecMarge.length > 0
    ? avecMarge.reduce((s: number, m: any) => s + (m.margePct ?? 0), 0) / avecMarge.length
    : 0;
  const sorted        = [...avecMarge].sort((a: any, b: any) => (b.margePct ?? 0) - (a.margePct ?? 0));
  const enAlerte      = avecMarge.filter((m: any) => m.statut === "ALERTE");
  const incomplets    = marges.filter((m: any) => m.source === "AUCUN");
...
export async function getMargesParCategorie(companyId: string) {
  const beurreMP = await prisma.matierePremiere.findFirst({
    where:   { companyId, nom: { contains: "beurre", ...modeInsensible }, actif: true },
...
export async function simulerImpactHausse(companyId: string, variations: {
  ...
  const nouveauxPrix = new Map<string, any>(mps.map((mp: any) => {
    const variation    = variations.find((v: any) => v.mpId === mp.id);
  ...
  const beurreMP = await prisma.matierePremiere.findFirst({
    where:   { companyId, nom: { contains: "beurre", ...modeInsensible } },
        ...
        recette: {
          ...
          ingredients: p.recette.ingredients.map((ing: any) => ({
            ...
            mp: nouveauxPrix.has(ing.mp.id)
              ? { ...ing.mp, prixAchat: (nouveauxPrix.get(ing.mp.id) as any)!.nouveauPrix }
              : ing.mp,
          })),
        },
      };
