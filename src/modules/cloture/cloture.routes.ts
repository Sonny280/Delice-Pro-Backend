...
//
// 2. RETIRÉ — confirmation manuelle des lots DLV expirés. Décision
//    prise ensuite : plus de suivi automatique par lot du tout,
//    les invendus sont désormais saisis manuellement à la clôture,
//    pour chaque produit, sans détection ni suggestion pré-calculée.
//
...
async function garantirPlageParDefaut(companyId: string) {
  const existantes = await prisma.plageHoraire.count({ where: { companyId } });
  if (existantes === 0) {
    await prisma.plageHoraire.create({
...
router.get("/plages", async (req: Request, res: Response) => {
  ...
  const plages = await prisma.plageHoraire.findMany({
...
router.post("/plages", requireRole(["ADMIN", "RESPONSABLE"]), async (req: Request, res: Response) => {
  ...
  const { companyId } = req.user!;
  const dernier = await prisma.plageHoraire.findFirst({
    where: { companyId }, orderBy: { ordre: "desc" },
  });
  const plage = await prisma.plageHoraire.create({
...
router.put("/plages/:id", requireRole(["ADMIN", "RESPONSABLE"]), async (req: Request, res: Response) => {
  ...
  const data = schema.parse(req.body);
  await prisma.plageHoraire.updateMany({
...
router.delete("/plages/:id", requireRole(["ADMIN", "RESPONSABLE"]), async (req: Request, res: Response) => {
  const restantes = await prisma.plageHoraire.count({
...
// ─── GET /api/cloture/produits — Produits pour la saisie des invendus ────────
// RETIRÉ — suggestions automatiques basées sur des lots DLV. Décision :
// plus de suivi par lot, les invendus sont saisis manuellement à la
// clôture pour chaque produit, sans suggestion pré-calculée.
...
router.get("/statut", async (req: Request, res: Response) => {
  ...
  const plage = plageHoraireId
    ? await prisma.plageHoraire.findFirst({ where: { id: plageHoraireId, companyId, actif: true } })
    : await prisma.plageHoraire.findFirst({ where: { companyId, actif: true }, orderBy: { ordre: "asc" } });
...
router.post("/journee", requireRole(["ADMIN", "RESPONSABLE", "CAISSIER"]), async (req: Request, res: Response) => {
  ...
  const { plageHoraireId, invendus, pertes, fondCaisse, ecartFond, notes } = schema.parse(req.body);

  const plage = await prisma.plageHoraire.findFirst({
  ...
  // RETIRÉ — traitement des lots DLV confirmés. Il n'existe plus de
  // lots à confirmer ; on part directement sur les invendus manuels.
...
router.delete("/:id", requireRole(["ADMIN", "RESPONSABLE"]), async (req: Request, res: Response) => {
  ...
  await logAudit({
    ...
    details: JSON.stringify({
      ...
      motif: (req.body as any)?.motif ?? "Non précisé",
    }),
  });
